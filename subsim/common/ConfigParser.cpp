...
Config ConfigParser::parseConfig(const ParseResult& parse)
{
    ...
    for (auto it = range.first; it != range.second; ++it)
    {
        for (auto keyValIt = it->second.cbegin(); keyValIt != it->second.cend(); ++keyValIt)
        {
            ...
            if (key == "torpedo_damage")
            {
                sstream >> result.torpedoDamage;
            }

            if (key == "mine_damage")
            {
                sstream >> result.mineDamage;
            }

            if (key == "collision_damage")
            {
                sstream >> result.collisionDamage;
            }
